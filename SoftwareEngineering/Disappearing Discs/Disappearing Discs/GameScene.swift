...
class GameScene: SKScene
{
    ...
    func spawnNewDisc()
    {
        ...
        let disc2 = SKSpriteNode(imageNamed: "Disc\(randomImageNumber)")
        disc.zPosition = 2
        disc.name = "discObject"
        disc2.zPosition = 2
        disc2.name = "disc2Object"
        ...
        self.addChild(disc)
        disc2.position = CGPoint(x: randomX, y: randomY)
        self.addChild(disc2)
        
        disc.run(SKAction.sequence([
            SKAction.scale(by: 0, duration: 3.0),
            playGameOverSoundEffect,
            SKAction.run(runGameOver)
            ]))
        
        disc2.run(SKAction.sequence([
